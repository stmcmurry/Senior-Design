...
GUI coordinates currently based off of a Samsung S5 Phone that is casted onto the laptop screen
...
class subMovement:
    ...
    def moveLeft(self, duration):
        ...
        mouse = Controller()
        mouse.press(Button.left) #mouse clicks
        pyautogui.dragTo(221, 700, duration=1.5) #mouse drags to new coordinate in 1.5s
        time.sleep(duration) #this time function ensures that the mouse is held for the alloted duration
        mouse.release(Button.left) #mouse is then released
        backToGUI() #GUI pops back up for the next command
...
#A class that controls camera functionality and lights  
class Camera:
    def __init__(self):
         self.power = True
         self.record = False
